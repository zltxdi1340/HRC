...
def _run_one_env(env_id: str, T: int, H: int, delta: int, tau: float, min_support: int, seed: int, save_bars: bool, out_dir: Path) -> TaskResult:
    ...
    # 需要你在 hrc_stage1.py 里已经有 detect_roots_by_baseline
    CS = set(hrc.detect_roots_by_baseline(
        env, ACTIVE, T=80, H=H, delta=delta, seed=seed + 999, rho=0.4
    ))
    IS: Set[str] = set()
    ...
    while ("success" not in IS) and len(CS) > 0:
        ...
        refined = hrc.refine_direct_edges(DI, env_id, IS, ACTIVE, base, tau=tau, min_support=min_support)
        for g in ACTIVE:
...
def main():
    ...
    ap.add_argument("--no_overrides", action="store_true", help="Disable per-env parameter overrides (DoorKeyBall needs larger H/delta to reveal door_open->at_goal).")
    ...
    # per-env overrides (only UnlockPickup)
    overrides = {
        "MiniGrid-UnlockPickup-v0": dict(T=180, H=600, delta=240, min_support=25, tau=0.01),
    }
    # 如果你想保留 --no_overrides 这个开关，可以加：if args.no_overrides: overrides = {}

    for env_id in args.env_ids:
        print("[run]", env_id)

        cfg = dict(T=args.T, H=args.H, delta=args.delta, tau=args.tau, min_support=args.min_support)
        cfg.update(overrides.get(env_id, {}))

        results.append(_run_one_env(
            env_id=env_id,
            T=cfg["T"], H=cfg["H"], delta=cfg["delta"],
            tau=cfg["tau"], min_support=cfg["min_support"],
            seed=args.seed, save_bars=args.bars, out_dir=out_dir
        ))
